...
- `#current-projects`: drei textbetonte Projektbereiche für Adeltuner, WorldBuilder und ScoreWriter. Kleine, leicht gedrehte Papierkärtchen ergänzen die Texte am Rand; bei Adeltuner überlappen zwei Bilder. Auf Mobilgeräten stehen die Kärtchen kompakt unter dem Text. `src/lib/current-projects.ts` enthält die deutschen/englischen Beschreibungen und Bildzuordnungen; `src/components/current-projects.tsx` und `src/app/current-projects.css` steuern das responsive Layout. Screenshots lassen sich per Klick im Original öffnen. Die vier Dateien liegen in `public/assets/projects/`, siehe README dort.
...
- `public/assets/catlogo.svg`: bereitgestellter Origami-Katzenkopf, als gemeinsames Logo in Navigation, Footer und Zettel-Titelblatt sowie als Favicon eingebunden. Der Theme-Filter passt das Logo an den Hintergrund an; die Wortmarke „karigami.“ bleibt echter Text in Manrope. `apple-touch-icon.png` ist die auf Papierweiß gerenderte Variante für den Homescreen. Die bisherigen SVG-Logos bleiben als Referenz erhalten.
